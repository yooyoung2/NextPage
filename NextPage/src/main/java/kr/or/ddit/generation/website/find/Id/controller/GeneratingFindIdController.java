...
@Controller
@RequestMapping( value = "generation/{id}/find/id" )
public class GeneratingFindIdController {

	private final GeneratingFindIdService service;
	

	@GetMapping
	public String generationFindId(
				HttpSession session 
			) {
		
		log.debug( "============= GeneratingFindIdController - generationFindId() [ACTIVATED] =============" );
		...
		if( templateId != null ) {
			...
			viewName = "generation/" + templateId + "/nomenu/38_generatingFindId/generatingFindId";
		}else {
		...
		log.debug( "============= GeneratingFindIdController - generationFindId() [DEACTIVATED] =============" );
		return viewName;
	}
	 ...
	 @PostMapping
	   public String generationFindIdsendEmail( 
         ...
         int rand=(int)(random*899999)+100000;
	      ...
	      sch.setMemNm(schNm);
	      sch.setMemEmail(schEmail);
	      sch.setSchId((String)session.getAttribute("id"));
	      SchMemberVO member=service.getSchInfo(sch);
	      
	     String wantSession=(String)session.getAttribute("id");
	      ...
	      try {
	         ...
	         // Subject
	         // 메일 제목
	         message.setSubject("제네레이션 ["+session.getAttribute("id")+"] 아이디 찾기 메일입니다!");
	      ...
	      session.setAttribute("findIdNumberData", member.getMemId());
	      model.addAttribute("msg","이메일 전송에 성공하였습니다!");
	      model.addAttribute("url","/generation/"+wantSession+"/find/id/number");
	      return "schoolManager/106_teacherInsert/success";
	      }
	      else
	      {
	    	  model.addAttribute("msg","입력된 학교명, 이메일을 찾을 수 없습니다!");
		      model.addAttribute("url","/generation/"+wantSession+"/find/id");
	     	  return "schoolManager/106_teacherInsert/success";
