<%@page import="kr.or.ddit.vo.ClassMemVO"%>
<%@page import="kr.or.ddit.vo.ClassMemVO"%>
<%@page import="kr.or.ddit.vo.ClassMemVO"%>
...
.input-file-button{
  ...
  cursor: pointer;
	...
	<%
	int check=0;
List<ClassMemVO> list=(List)request.getAttribute("list");
String fileName=(String)request.getAttribute("fileName");


if(list!=null)
{
	List<ClassMemVO> poiList = new ArrayList<ClassMemVO>();
	ClassMemVO poi=new ClassMemVO();

	%>
	<tr>
		<th><h3>파일이름: <%=fileName %></h3><br><h5>받아온 데이터 수: <%=list.size() %></h5></th>
		<td colspan="2">
			<form method="post" action="<%=request.getContextPath() %>/school/manager/studentClassRealInsert" >
	...
	<%
					...
					/* if(userCheck1>checkInt1||userCheck3>checkInt3||!"-".equals(userCheck2))
					{
						...
						logicalViewName = "schoolManager/108_studentClassInsert/success";
...
<%-- 			alert("적절하지 않은 형식의 데이터가 들어있습니다! 오류개수: "+<%=check%>); --%>
			</script>
		<%
	}
	%>
	...
	if(check!=0&&err==0)
	{
		...
		Swal.fire({
		  icon: 'error',
		  title: 'Error',
		  text: '생성된 학년-반 범위를 초과하는 데이터가 있습니다! 오류개수: '+<%=check%>,
		});
...
document.getElementById('file2').addEventListener('input', (event) => {
  ...
  f.setAttribute('action', '/NextPage/school/manager/studentClass/poi/');
