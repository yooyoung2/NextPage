...
List<String> list=(List<String>)request.getAttribute("situationList");
int index=0;
%>
			...
			<div>
				<div id="inputTags" style="margin-left: 80%;    margin-bottom: 1%;">
					<button type="button" class="btn btn-primary"
						onclick="location.href='<%=request.getContextPath()%>/school/manager/teacherEachInsert'">개별등록</button>
							...
							<table>
								...
								<tbody>
									<tr>
										...
										<th><br><a href="<%=request.getContextPath() %>/school/manager/teacher/fileInsert"><button style="margin-left:50px;" class="btn btn-info" >파일첨부</button></a></th>
...
<!-- 모달창을 띄우고 모든 이벤트를 적용하는 js파일 -->
	...
	let test = $("#test2").on("click", function(event){
	    ...
	    f.setAttribute('action', '/NextPage/school/manager/teacher/fileInsert');
...
function sendPost(email){
    ...
    f.setAttribute('action', '/NextPage/school/manager/teacher/update/');
    document.body.appendChild(f);
    f.submit();
}
