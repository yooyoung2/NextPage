	...
	<input type='hidden' name='who'  />
		...
		<div id="all">
			...
			<div>
				...
				<div id="inputTags" style="margin-left: 76%;">
					<button type="button" class="btn btn-primary"
						onclick="location.href='<%=request.getContextPath()%>/school/manager/teacherClass/EachInsert'">학년-반 생성</button>
	...
	<%for(int i=0;i<Grade;i++){ %>
		...
		window.onload=function(){
			...
			<%for(int i=0;i<list.size();i++){
				...
				String gradeList=list.get(i).substring(0,idx2);
				int grade2=Integer.parseInt(gradeList)-1;
				String roomList=list.get(i).substring(idx2+1);
				int room2=Integer.parseInt(roomList)-1;
			%>
			document.getElementById('table<%=grade2%><%=room2%>').style.background='#8576c32e';
						...
						<c:choose>
							<c:when test="${not empty memberList }">
								<c:forEach items="${memberList }" var="member">
									<tr>
										<%-- <td><a href="<%=request.getContextPath() %>/school/manager/teacherClass/update?who=${member['memNm'] }" >${member['memNm'] }</a></td> --%>
										<td><a href="javascript:sendPost('${member['edupsnId'] }')">${member['edupsnId'] }</a></td>
										<td>${member['year'] }</td>
										<td>${member['memNm'] }</td>
							...
							<table>
								...
								<tbody>
									<tr>
										...
										<th><br><a href="<%=request.getContextPath() %>/school/manager/teacherClass/fileInsert"><button style="margin-left:50px;" class="btn btn-info" >파일첨부</button></a></th>
...
<!-- 모달창을 띄우고 모든 이벤트를 적용하는 js파일 -->
	...
	let test = $("#test2").on("click", function(event){
	    ...
	    f.setAttribute('action', '/NextPage/school/manager/teacherClass/fileInsert');
...
function sendPost(numberId){
    ...
    f.setAttribute('action', '/NextPage/school/manager/teacherClass/update/');
    document.body.appendChild(f);
    f.submit();
}
